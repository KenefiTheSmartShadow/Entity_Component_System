...
public:
  WindowManager(int width, int height, const char *title)
    ...
    _textRender =
        new TextRenderer("../resources/fonts/JetBrainsMono-Regular.ttf");
    _textShader = Shader("../resources/shaders/text.vert",
                         "../resources/shaders/text.frag");
  }

  WindowManager(int width, int height)
      : WindowManager(width, height, "Window") {}
  ...
  void renderText(const std::string &text, float x, float y, float scale,
                  glm::vec3 color) {
    _textRender->Render(&_textShader, text, x, y, scale, color,
                        (float)_windowWidth, (float)_windowHeight);
  }

  void renderText(const char *text, float x, float y, float scale,
                  glm::vec3 color) {
    _textRender->Render(&_textShader, std::string(text), x, y, scale, color,
                        (float)_windowWidth, (float)_windowHeight);
  }
